          ...
          <div class="flex flex-col gap-4 md:gap-6">
            <div class="space-y-2 text-center md:text-start">
              ...
              <h1 class="text-2xl md:text-3xl font-bold tracking-tight">
                {{ pageData?.title || t('footer.privacyPolicy') }}
              </h1>
            </div>
...
const pageData = ref<PrivacyPolicyResponse['data'] | null>(null)
...
useHead(() => ({
  title: pageData.value?.title || t('footer.privacyPolicy'),
}))
...
const fetchPrivacyPolicy = async () => {
  ...
  const { data, error } = await apiFetch<PrivacyPolicyResponse>('/privacy-policy')
