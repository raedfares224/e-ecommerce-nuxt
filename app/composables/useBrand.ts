...
export const useBrand = () => {
  ...
  const category = useState<CategoryDetail | null>('brand-detail', () => null)
  const products = useState<CategoryProduct[]>('brand-products', () => [])
  const topRatedProducts = useState<TopRatedProduct[]>('brand-top-rated', () => [])
  const filterCategories = useState<FilterCategory[]>('brand-filters', () => [])
  const filterBrands = useState<FilterBrand[]>('brand-brands', () => [])
  
  const loading = useState<boolean>('brand-loading', () => false)
  const productsLoading = useState<boolean>('brand-products-loading', () => false)
  const error = useState<string | null>('brand-error', () => null)
  
  // Pagination
  const currentPage = useState<number>('brand-current-page', () => 1)
  const lastPage = useState<number>('brand-last-page', () => 1)
  const perPage = useState<number>('brand-per-page', () => 12)
  const total = useState<number>('brand-total', () => 0)
  ...
  const fetchCategoryDetails = async (slug: string) => {
    try {
      ...
      const response = await get<CategoryDetailResponse>(`/brands/${slug}`, {
        page: 1
      })
      ...
      if (response && response.data && Array.isArray(response.data)) {
        ...
        category.value = null
        console.warn('Brand details response is empty or invalid')
      }
    } catch (err: any) {
      error.value = err.message || 'Failed to load brand details'
  ...
  const fetchCategoryProducts = async (
    ...
    try {
      ...
      if (response && response.data) {
        ...
        products.value = []
        console.warn('Brand products response is empty or invalid')
      }
    } catch (err: any) {
